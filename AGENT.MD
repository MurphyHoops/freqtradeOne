...
- 信号准入: 当前实现为统一 `min_score`（可关闭）；未命中即拒绝并记录原因。TierAgent 进一步按 min_raw_score/min_rr/min_edge 过滤。Tier 白名单仅基于 recipe，entry 级过滤由 `StrategySpec.entries` 定义，已移除 tier 级 `allowed_entries`。
