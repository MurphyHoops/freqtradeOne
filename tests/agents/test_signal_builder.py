...
def test_builder_emits_long_candidates_when_conditions_met():
    cfg = V29Config()
    row = _row(close=97.0, ema_fast=100.0, ema_slow=95.0, rsi=20.0, adx=30.0)
    ...
    for c in cands:
        assert c.sl_pct > 0
        assert c.tp_pct > c.sl_pct
        assert c.rr_ratio >= 1.2


def test_builder_emits_short_candidate_for_trend_short():
    cfg = V29Config()
    row = _row(close=102.0, ema_fast=100.0, ema_slow=105.0, rsi=40.0, adx=30.0)
    cands = build_candidates(row, cfg)
    kinds = {c.kind for c in cands}
...
def test_builder_handles_timeframe_specific_spec(monkeypatch):
    ...
    spec = SignalSpec(
        ...
        sl_fn=lambda bag, _: bag["ATR_PCT"],
        tp_fn=lambda bag, _: bag["ATR_PCT"] * 2.0,
