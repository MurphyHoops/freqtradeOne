...
def test_builder_emits_long_candidates_when_conditions_met():
    ...
    for c in cands:
        assert c.rr_ratio > 0
        assert c.win_prob > 0
        assert c.exit_profile == cfg.default_exit_profile
...
def test_builder_emits_short_candidate_for_trend_short():
    ...
    assert short.exit_profile == cfg.default_exit_profile
...
def test_builder_handles_timeframe_specific_spec(monkeypatch):
    ...
    cfg.default_exit_profile = "tf_profile"
    ...
    cfg.enabled_signals = cfg.enabled_signals + ("tf_long",)
    spec = SignalSpec(
        name="tf_long",
        direction="long",
        squad="TF",
    ...
    assert cands[0].exit_profile == cfg.default_exit_profile
